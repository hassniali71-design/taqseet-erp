...
**قاعدة عامة تنطبق على كل ما هو موجود حاليًا:** كل شيء **عدا تسجيل الدخول** لسه مبني على **Mock data-store** (`src/lib/data-store.ts`، localStorage + subscribe/emit). كل الجداول/الحقول مطابقة بالاسم لـ`docs/ERP_SaaS_Requirements.md §118` و`supabase/migrations/*.sql` — الانتقال لاحقًا **drop-in replacement** لأجسام الدوال في `data-store.ts`، مش إعادة بناء. **⚠️ طالما البيانات (عدا الدخول) Mock، لا Phase "جاهزة للإنتاج" بمعايير §134** — RLS/Tenant isolation/Server-side authorization الحقيقيين (§133) لا يتحققوا إلا بعد ربط باقي `data-store.ts` فعليًا.

**✅ تسجيل الدخول بقى Supabase Auth حقيقي** — مشروع Supabase حقيقي مربوط (المستخدم عمله على حسابه، مفاتيحه في `.env`/`VITE_*` محليًا عنده، مش في هذا الريبو). `signIn()`/`signOut()` في `data-store.ts` بيستخدموا `supabase.auth.signInWithPassword`/`signOut` فعليًا (`src/lib/supabase-client.ts` — عميل متصفح بالـanon key فقط) — تشفير الباسورد حقيقي من Supabase نفسه لأول مرة، مش نص صريح. **تصميم "جسر" متعمد**: `signIn()` بعد نجاح المصادقة بيجيب `tenant_id`/`is_platform_owner` من صف `users` الحقيقي (عبر `auth_user_id`) ويكتبهم لنفس `localStorage` Session القديمة بنفس الـshape — فـ`getSession()` وكل الـ28 route اللي بيستخدموا `useSession`/`useRequireSession` **ما اتغيّروش خالص**. عشان البيانات التجريبية (لسه Mock) تفضل شغالة عادي بعد تسجيل دخول حقيقي، `DEMO_TENANT_ID`/`PLATFORM_TENANT_ID` ومعرّفات المستخدمين التجريبيين في `data-store.ts` اتغيّروا من نصوص عشوائية (`"tenant_demo"`) لنفس الـUUIDs الحقيقية المزروعة يدويًا في قاعدة بيانات المستخدم (migration `0010_add_is_platform_owner.sql` + INSERT سكريبت يدوي، مش migration دائم — التفاصيل اتسجلت في المحادثة مع المستخدم، مش في ملف SQL منفصل لأنه بيانات محل واحد بعينه مش schema عام).
**لسه ناقص عمدًا:** باقي `data-store.ts` (عملاء/منتجات/مبيعات/تقسيط...) لسه Mock بالكامل — بيانات مش متزامنة بين الأجهزة حتى بعد تسجيل دخول حقيقي. `getSession()` لسه بيقرأ `localStorage` بس (مفيش تحقق حي من صلاحية جلسة Supabase). `provisionTenant()` (إنشاء عميل جديد من `/platform`) لسه بيعمل مستخدم Mock بس — عميل جديد من غرفة تحكم المنصة لسه مش هيقدر يسجّل دخول حقيقي. **لا تدّعي اكتمال ربط Supabase — جزء واحد بس (الدخول) بقى حقيقي.**

**بيانات الدخول التجريبية:** `owner@demo.local` / `owner123` و`platform@hesba.local` / `hesba123` — دلوقتي حسابات Supabase Auth حقيقية (مش Mock)، لازم تتغيّر كلمات السر دي قبل أي تسليم فعلي للعميل.
...
- **✅ جداول البيانات الكبيرة بقت Scroll داخلي**: أي جدول قوائم ممكن يكبر مع الوقت (عملاء، منتجات، سجل تدقيق، تحصيل، موردين، حركات خزينة، مستخدمين، توصيلات، مصروفات، قيود، تقارير، وجدول Tenants في `/platform`) بقى محدود بـ`max-h-[26rem] overflow-y-auto` + `<thead className="sticky top-0 ...">` — الكارت بيفضل بحجمه، مش بيكبر بلا حدود مع تراكم البيانات. اتحقق فعليًا بمتصفح حقيقي بحقن ٤٠ صف تجريبي في `/customers`. نمط ثابت لأي جدول قوائم جديد يُضاف مستقبلاً.
...
- **Phase 2 — Products & Inventory (تكملة فوق `/products`) — منفّذة:**
  ...
  - **✅ Categories/Brands بقت قوائم مُدارة حقيقية** (`ProductCategory`/`ProductBrand`، `registerProductCategory`/`registerProductBrand` idempotent بالاسم) — الفارق الفئة/الماركة في فورم `/products` بقى مربوط بـdatalist من قوائم حقيقية قابلة للإدارة من `/settings` (إضافة + تعطيل، بدون حذف نهائي §20)، بدل ما يبقى نص حر بحت. `Product.category`/`Product.brand` **لسه** نص عادي (مش Foreign Key) — قرار مقصود، مذكور في تعليق `ProductCategory`/`ProductBrand` بـ`types/index.ts`. Migration: `0009_product_categories_brands.sql`.
  - **لسه ناقص عمدًا:** Multiple Units (تحويل وحدات، زي "كرتونة = ١٢ قطعة")، تسعير خاص بعميل (Customer-specific pricing) — لم يُنفَّذا بعد، مش مذكورين في أي مكان تاني كمنفّذين.
...
### Phase 9 — SaaS Control Center — منفّذة بالكامل (بما فيها عزل البيانات الحقيقي)
...
- **تفعيل/تعليق** (`setTenantStatus`، موجودة من قبل) و**تجديد اشتراك ٣٠ يوم** (دالة `extendTenantSubscription` — تمدّ من تاريخ الانتهاء الحالي لو لسه ساري، أو من الآن لو منتهي بالفعل).
- بوابتا دخول منفصلتان فعلياً (انظر قسم الهوية البصرية) — كل باب يرفض نوع الحساب التاني برسالة واضحة.
- **✅ عزل بيانات الأعمال لكل تينانت — حقيقي فعلاً الآن، مش ملاحظة تحذيرية.** `scopeToTenant<T extends {tenant_id:string}>(rows, tenantId)` هيلبر مشترك، و٢٦ دالة `get*()` (عملاء، منتجات، مبيعات، عقود تقسيط، أقساط، موردين، خزينة، مصروفات، قيود...) بقى ليها باراميتر اختياري `tenantId?: string`. استدعاء بدون باراميتر (زي ما كل دوال الـMutation الداخلية لسه بتعمل) بيرجّع المجموعة كاملة بدون فلترة — صفر خطر على منطق الكتابة الموجود. صفحات الـroutes بقت تمرّر `session.tenant_id` صراحة لغرض العرض. `getWarrantyInfo` (بحث حر برقم سيريال) بقى `tenantId` فيها **إلزامي مش اختياري** — كانت ثغرة بحث عبر-تينانت حقيقية اتقفلت فعلاً.
  - **باگ حرج اتصلح أثناء التنفيذ:** أول تعديل شامل (sed) لتحويل `DEMO_TENANT_ID` الحرفي لـ`getCurrentTenantId()` كان طال بالغلط دوال الـ`seed*()` الخمستاشر كمان — يعني أول تينانت جديد يزور صفحة (أي صفحة، مش بس الديمو) كان بيـ"يخطف" بيانات الديمو الأساسية ويوسمها بمعرّفه هو. اتصلح بإرجاع كل دوال الـseed لتوسم بياناتها الثابتة بـ`DEMO_TENANT_ID` الحرفي دايمًا، بغض النظر عن مين بيزور الصفحة أول مرة.
  - **✅ اتحقق فعليًا بمتصفح حقيقي (Playwright)، 10/10 نقاط + فحوصات إضافية على المنتجات/الموردين/الخزينة:** تينانت جديد بيشوف صفر عملاء/منتجات/موردين عند أول زيارة (مش بيانات الديمو)، عملاء تينانت الديمو ما بيتأثروش، وبالعكس، وغرفة تحكم المنصة بتشوف الاتنين.
- **✅ Support Access حقيقي** (`/platform/support/$tenantId`, `src/routes/platform_.support.$tenantId.tsx`): مش انتحال هوية حرفي — سبب إلزامي قبل الدخول (بيتسجل في سجل تدقيق **التينانت المستهدف نفسه**، مش سجل المنصة، عشان صاحب المحل يقدر يشوف كل مرة فريق المنصة دخل بياناته وليه)، شاشة قراءة فقط (عدد عملاء/منتجات/مبيعات/عقود نشطة/أقساط متأخرة/رصيد خزينة) + آخر ١٠ عمليات دخول دعم مسجّلة لنفس التينانت.
- لا ربط Supabase حقيقي (خطوة منفصلة تمامًا، تنتظر بيانات اعتماد من المستخدم) — العزل الحالي تطبيقي (Application-layer) فوق Mock؛ عند ربط Supabase حقيقي لازم RLS كطبقة حماية ثانية، مش بديل عن هذا العزل ولا معتمد عليه وحده.

**الخلاصة الصريحة:** Phase 9 مكتملة فعليًا الآن — دورة حياة Tenant كاملة (إنشاء/تفعيل/تعليق/تجديد)، إحصائيات، توليد بيانات دخول، عزل بيانات حقيقي، وShاشة Support Access. الحاجة الوحيدة الباقية خارج نطاق "Mock جاهز للتجربة" هي ربط Supabase حقيقي (RLS فعلية على مستوى قاعدة البيانات) — مش جزء من هذه الدفعة، بيستنى بيانات اعتماد من المستخدم.
